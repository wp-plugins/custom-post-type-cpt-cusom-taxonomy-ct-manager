...
function rtm_front_page(){ ?>
    ...
    <h2><?php _e( 'Hello Pal, Start Using rtManager and Have Fun !','cpt-ct' ) ?></h2><?php
}
...
function rtm_create_post_type(){
    ...
    if( isset( $_POST['rt_submit'] ) && !empty($_POST['rt_cpt_slug']) && !empty($_POST['rt_cpt_name'])  ){
        ...
        echo update_option( 'rt_cptmn', $available_cpt );
...
function rtm_common_form( $is_cpt_page = true ){ ?>
    <div id="rtm-manager-form">
        <form method="post" action="" >
                ...
                <div class="fields-container">
                    <label for="rt_cpt_slug"><?php _e( 'Slug for new post type','cpt-ct' ); ?></label>
...
function rtm_create_taxonomy(){
    ...
    if( isset( $_POST['rt_submit'] ) && !empty($_POST['rt_cpt_slug']) && !empty($_POST['rt_cpt_name'])  ){
        ...
        echo update_option( 'rtm_custom_tax', $available_ct );
