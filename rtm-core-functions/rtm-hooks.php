...
function rt_add_styles_and_scripts(){
    wp_enqueue_script( 'rtm', plugin_dir_url().'cpt-ct-manager/js/rtm.js', array(), false, true );
    
    wp_enqueue_style('rtm-style', plugin_dir_url().'cpt-ct-manager/styles/rtm-style.css');
}
...
function rt_create_menu(){
    ...
    $rtmanager = add_menu_page( __( 'CPT CT Manager','cpt-ct' ), __( 'CPT CT Manager','cpt-ct' ), 'administrator', 'cptctm', 'rtm_front_page', plugins_url('cpt-ct-manager/images/icon.ico') );
    add_submenu_page( 'rtm', __( 'Create CPT','cpt-ct' ), __('Create Custom Post Type','cpt-ct'), 'administrator', 'cptctm-create-cpt', 'rtm_create_post_type' );
    add_submenu_page( 'rtm', __('Create CT','cpt-ct'),  __('Create Custom Taxonomy','cpt-ct'), 'administrator', 'cptctm-create-ct', 'rtm_create_taxonomy' );
