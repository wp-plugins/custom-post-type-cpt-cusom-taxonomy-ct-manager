...
function rtm_front_page(){ ?>
    ...
    <h2 class="cptm-welcome-message"><?php _e( 'Hello Pal, Start Using Custom Post Type and Taxonomy (CPT CT) Manager and Have Fun !','cpt-ct' ) ?></h2>
    <h3><?php _e('What you can do with this plugin !','cpt-ct'); ?></h3>
    <ol>
        <li><?php _e('Create custom post types and let this plugin do all the necessary codes for you.','cpt-ct') ?></li>
        <li><?php _e('Create custom post types and associate existing taxonomies to it.','cpt-ct') ?></li>
        <li><?php _e('Create taxonomies and let this plugin do all the necessary codes for you.','cpt-ct') ?></li>
        <li><?php _e('Create taxonomies and associate existing post types to it.','cpt-ct') ?></li>
    </ol>
    <p class="cptm-start-using"><strong><?php _e('For any queries or support kindly visit following links and post your feedback','cpt-ct'); ?></strong></p>
    <ul class="cptm-nav-link">
        <li><a target="_blank" href="<?php echo 'http://wordpress.org/support/plugin/custom-post-type-cpt-cusom-taxonomy-ct-manager'; ?>"><?php _e('Cuctom post type & taxonomy GUI manager wordpress support forum','cpt-ct'); ?></a></li>
        <li><a target="_blank" href="<?php echo 'http://alnobody70.wordpress.com'; ?>"><?php _e('Cuctom post type & taxonomy GUI manager wordpress official support website','cpt-ct'); ?></a></li>
    </ul><?php
}
...
function rtm_create_post_type(){
    ...
    if( isset( $_POST['rt_submit'] ) && !empty($_POST['rt_cpt_slug']) && !empty($_POST['rt_cpt_name'])  ){
        ...
        echo '<p>Congratulations Friend...!!! <br/><br/>Custom Post Type Created Successfully. :D</p>';
...
function rtm_common_form( $is_cpt_page = true ){ ?>
    <div id="rtm-manager-form">
        <form method="post" action="" >
                ...
                if($is_cpt_page){ ?>
                    <div class="fields-container">
                        <label for="rt_menu_icon"><?php _e( 'Menu Icon URL ( 16px X 16px ) [optional]','cpt-ct' ); ?></label>
                ...
                <div class="fields-container">
                    <?php $button_value = ($is_cpt_page) ? 'Create custom post type' : 'Create taxonomy'; ?>
                    <input type="submit" name="rt_submit" id="rt-submit" value="<?php echo $button_value; ?>" />
                </div>
                <p class="cptm-flush-permalink"><strong><?php _e('Note : ','cpt-ct'); ?></strong><?php _e('Please save the permalink once, after creating post type or taxonomy','cpt-ct') ?></p>
        </form>
    </div><?php
}
